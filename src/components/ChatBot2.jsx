...
async function fetchFAQs() {
  try {
    const response = await axios.get("http://localhost:5000/api/getJsonData/?fileKey=faqs.json");
    console.log("faqs", response.data); // Access data property
    faqs = response.data.data
    console.log("faqs2", faqs); //
  } catch (error) {
...
const messageMap = {
  start: {
    ...
    options: [
      ...
      { key: "faq", text: "FAQs" }, // Add FAQ option
  ...
  "1": {
    ...
    steps: [
      "Click on this link: https://www.rufrent.com/user/postProperties",
      "Complete Signup/Login Process",
      "Click on [Post Property Free] Button",
      "Fill All Required Details and Click On Submit Button",
      "Property Posting is Successful, Will be Visible After Approvals",
    ],
  ...
  "2": {
    ...
    steps: [
      "Visit Link: https://www.rufrent.com/user",
      "Apply required filters to get curated properties",
      "Click on Connect to Relationship Manager Button for Further Assistance.",
    ],
    options: [{ key: "9", text: "Back to Main Menu" }],
  },
  "3": {
...
const Chatbot = ({ onClose }) => {
  ...
  const handleBotMessageClick =async (key) => {
    console.log('handleBotMessageClick',key);
    if (key.startsWith("faq-")) {
     
      console.log("vikas1",key, messageMap[key])
      // Handle FAQ question click
      const index = parseInt(key.split("-")[1], 10);
      const selectedFAQ = faqs[index];
      setMessages((prev) => [
        ...prev,
        { type: "user", text: `Q: ${selectedFAQ.question}` },
        { type: "bot", text: `A: ${selectedFAQ.answer}` },
        { type: "bot", text: "Back to Main Menu", key: "9", clickable: true },
        { type: "bot", text: "More FAQs", key: "faq", clickable: true },

      ]);
    } else if (messageMap[key]) {
      console.log("vikas2",key, messageMap[key])
      sendMessage(key);
    }
  };
